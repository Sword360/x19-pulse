...
export function AddServerModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const installCommand = `curl -sSL ${serverUrl}/install.sh | bash -s -- ${agentToken}`;
  const wslCommand = `PULSEOPS_SERVER="${serverUrl}" PULSEOPS_TOKEN="${agentToken}" python3 agent/pulseops-agent.py`;
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-2xl w-full p-6 shadow-2xl relative">
        ...
        <div className="flex items-center space-x-3 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Deploy PulseOps Agent to connect a new server</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-xs font-semibold text-indigo-400 uppercase tracking-wider flex items-center space-x-1.5">
                ...
                <span>1-Line Automatic Installer (Linux / Ubuntu / Debian / RHEL)</span>
              </span>
              <button
            ...
            <p className="text-xs text-slate-400">
              Run this single command on your Linux server terminal. It automatically downloads, configures, and registers the agent daemon:
            </p>
          ...
          {/* WSL / Quick Command Line Setup */}
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2">
            <span className="text-xs font-semibold text-slate-300 flex items-center space-x-1.5">
              <Terminal className="w-4 h-4 text-indigo-400" />
              <span>WSL / Manual Python Execution</span>
            </span>
            <p className="text-xs text-slate-400">
              To run the agent directly on WSL or Python environment:
            </p>
            <div className="bg-slate-900 border border-slate-800/80 p-3 rounded-lg font-mono text-xs text-slate-300 break-all select-all">
              {wslCommand}
            </div>
          </div>
        </div>
